...
function weightedAverage(state: PlannerState, metrics: {
  price: number;
  review: number;
  route: number;
  location: number;
  ...
  const score =
    (state.weights.price * metrics.price +
...
export function scoreFlights(state: PlannerState, flights: FlightResult[]): FlightResult[] {
  ...
  const scored = flights.map((flight) => {
    ...
    const comfortMetric =
      (flight.summary.includes("프리미엄") ? 0.95 : 0.7) +
...
export function scoreStays(state: PlannerState, stays: StayResult[]): StayResult[] {
